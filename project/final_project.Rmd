...
```{r getting_things_ready, echo=F, eval=F, message=F, warning=F, error=F}
library(ggplot2)
library(tidyverse)
library(dplyr)
library(readr)
...
dat_ <- RJSONIO::fromJSON(txt = path_json, flatten = TRUE) %>%
...
```{r mutations, echo=T, eval=F, message=F, warning=F, error=F}
dat_plus <- dat_ %>% 
...
```{r sources, echo=T, eval=F, message=F, warning=F, error=F}
test <- dat_plus %>% 
  filter(year >= 2017) %>% 
...
```{r analysis_prep, echo=T, eval=F, message=F, warning=F, error=F}
...
``` {r sentiment_prep, echo=T, eval=F}
